...
def run_gofood(start_date: str, end_date: str, outlet_filter: str = None, branch_filter: str = None):
    gofood_dir = os.path.join(os.path.dirname(__file__), "gofood-automation")
    ...
    print(f"\n{YELLOW}{BOLD}▶ GOFOOD PIPELINE{RESET}")
    ...
    if result.returncode == 0:
        print(f"\n{GREEN}✓ GoFood pipeline completed successfully.{RESET}")
        return True
    else:
        print(f"\n{RED}✗ GoFood pipeline exited with code {result.returncode}.{RESET}")
        return False


def run_gofood_baseline(start_date: str, end_date: str, outlet_filter: str = None, branch_filter: str = None):
    return run_gofood(start_date, end_date, outlet_filter, branch_filter)
...
def interactive_mode():
    ...
    if task_choice == "1":
        ...
        print(f"\n  {BOLD}Pilih Platform untuk Baseline:{RESET}")
        print(f"    {GREEN}[1]{RESET} Grab")
        print(f"    {MAGENTA}[2]{RESET} Shopee")
        print(f"    {YELLOW}[3]{RESET} GoFood")
        print(f"    {CYAN}[4]{RESET} Ketiganya (Grab + Shopee + GoFood)")
        print()

        while True:
            choice = input(f"  {BOLD}Pilihan (1/2/3/4) [4]:{RESET} ").strip()
            if not choice:
                choice = "4"
    ...
    # ─ Confirmation ─
    platform_label = {"grab": "Grab", "shopee": "Shopee", "gofood": "GoFood", "all": "Ketiganya (Grab + Shopee + GoFood)"}[platform]
...
Examples:
  ...
  python cli.py all   --start 2026-05-05 --end 2026-05-11
    ...
    if platform in ("gofood", "all"):
        if task_choice == "1":
            results["GoFood"] = run_gofood_baseline(start_date, end_date, outlet_filter=outlet, branch_filter=branch)
        else:
            results["GoFood"] = run_gofood(start_date, end_date, outlet_filter=outlet, branch_filter=branch)
                ...
                try:
                    ...
                    if not webhook_url or "GANTI_DENGAN_URL_ANDA" in webhook_url:
                        ...
                        for _, row in combined_df.iterrows():
                            ...
                            elif "go" in app or "gofood" in app:
                        ...
                        payload = {
                            ...
                            "order_go": format_order(order_go),
                            "omzet_gr": format_rp(omzet_gr),
                            "order_gr": format_order(order_gr),
                            "omzet_sf": format_rp(omzet_sf),
                            "order_sf": format_order(order_sf),
                            "total_omzet": format_rp(total_omzet),
                            "total_order": format_order(total_order)
                        }
